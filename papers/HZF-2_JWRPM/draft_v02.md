...
For tree (branching) topologies, junction nodes $j$ introduce multi-way coupling constraints. At junction $j$ connecting upstream pools $\mathcal{U}(j)$ and downstream pools $\mathcal{D}(j)$:
...
Memory footprint is minimal: even the 50-pool system requires only 3.2 MB of RAM, well within the CX2062's 8 GB capacity. Power consumption: the CX2062 draws 28 W at full load vs. 12 W idle. At 0.44% CPU utilization (B4), the incremental power for ParaQP is approximately 0.07 W—negligible for solar-powered remote sites.
...
__Figure 6.__ Coupling strength $\kappa$ as a function of pool length $L$ for typical canal parameters ($A_s = 50$–$500$ m², $\Delta t = 60$ s, $Q/R = 10$). Dashed lines indicate convergence thresholds for Jacobi ($\kappa < 1/\sqrt{2}$) and serial relaxation ($\kappa < 1$). Pools longer than 5 km have $\kappa < 0.1$; pools shorter than 2 km have $\kappa > 0.3$ (ADMM recommended).
