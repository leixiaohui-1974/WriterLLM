...
where $\alpha$ is the gate linearization coefficient, $\Delta h_{\text{ref}}$ is the nominal head difference across the gate at the operating point (typically 0.1–0.5 m for canal gates), and $\|Q_i\|/R_i$ is the tracking-to-control weight ratio. Figure 6 shows $\kappa$ as a function of pool length $L$ for typical canal parameters. For pools longer than 5 km ($A_s > 200$ m²), $\kappa < 0.1$ and all three strategies converge rapidly. For short pools ($L < 2$ km, $\kappa > 0.3$), ADMM is preferred due to its unconditional convergence guarantee.
...
*Proof*. The convex QP with $H_i \succ 0$ (ensured by $R_i \succ 0$) satisfies Boyd et al. (2011, §3.2) convergence conditions. The water network's tridiagonal coupling structure means each ADMM consensus update (Step 2) involves only 2 agents per edge, preserving locality. Warm-starting the dual variables $\mu_{ij}$ from the previous sampling step provides a near-feasible initial point that accelerates dual convergence. The optimal $\rho^*$ minimizes the condition number of the augmented system (Ghadimi et al., 2015).  $\square$
...
1. **Strategy selection**: If $N_p \leq 5$ and only single- or dual-core hardware is available, use Serial GS. For $N_p > 5$ on multi-core hardware (≥4 cores), compute $\kappa$ from Eq. 10 using the canal's physical parameters. If $\kappa < 0.5$, use Jacobi parallel (default recommendation); if $\kappa \geq 0.5$, use ADMM consensus.

2. **Early termination threshold**: Set $\epsilon_p = 0.001 \cdot Q_{\text{design}}$ (0.1% of design flow), corresponding to <0.1 mm water level impact for typical canal geometries. During commissioning, run 100 steps with machine-precision convergence ($\epsilon_p = 10^{-8}$) and verify that relaxing to the recommended threshold produces indistinguishable control performance.
...
ADMM convergence is robust within a factor of 3 around $\rho^*$ (wall time within 20% of optimal). The auto-tuning method (Wohlberg, 2017) consistently selects $\rho \in [0.7\rho^*, 1.5\rho^*]$, explaining its near-optimal performance (Figure 7).
