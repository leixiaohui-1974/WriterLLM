...
where $C_0, C_1, C_2$ are Muskingum routing coefficients derived from the Family β parameters ($K_i, X_i$). The ODD WARNING threshold is automatically raised from $\rho = 0.3$ to $\rho = 0.5$ during declared flood events, providing additional safety margin for reservoir level management.
