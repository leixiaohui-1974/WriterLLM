...
This work represents an early exploration of CHS-based autonomous control for cascade hydropower. Preliminary operator trials (2 weeks, 4 operators) at Shaoping revealed that operators adapted to the HDMPC recommendation interface within 2 days, with override rates decreasing from 45% (day 1) to 8% (day 14). Operators valued the ODD distance indicator $\rho(\mathbf{x})$ as a situational awareness tool. A formal training program (2-day classroom + 1-week supervised operation) is planned for full WSAL-2 deployment.
...
5. **Proactive maintenance scheduling**: The current HDMPC handles station outages reactively (e.g., S6: Luodixi offline). Proactive maintenance scheduling can be integrated into the Area Agent's 24-hour look-ahead by encoding planned outages as time-varying constraints, converting the MIQP to a time-varying formulation with known binary schedules that actually simplifies the optimization (no branch-and-bound needed for scheduled outages). Implementation is planned for the WSAL-3 transition phase.
