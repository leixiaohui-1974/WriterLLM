...
The MATLAB/Simulink dependency is the primary barrier to fully open-source deployment. Future work includes developing a Python/CasADi alternative for the DMPC design and code generation steps, which would eliminate the commercial software dependency.

__7.6  Open-Source Alternative: Python/CasADi Pathway__

The MATLAB/Simulink dependency (~$5,000/seat commercial) limits adoption, particularly in developing countries. An alternative pathway using Python and CasADi (Andersson et al., 2019) is under development. CasADi provides symbolic differentiation, QP solvers (qpOASES, OSQP), and native C code generation—the core capabilities needed for DMPC design and deployment. The barrier is the absence of a PLC-specific Structured Text converter equivalent to HydroRTP's custom converter. Estimated development effort: 6 person-months. Target: 2027 release. This will enable a fully open-source pipeline: HydroComponents (OpenModelica, free) + CasADi (Python, free) + PLC deployment (open converter).

__7.7  Digital Twin Pipeline__

The integrated toolchain effectively constitutes a digital twin pipeline for canal systems. The HydroComponents FMU serves as the digital twin plant model—a virtual replica of the physical canal running the same Saint-Venant equations with calibrated parameters. The DMPC controller runs in parallel as the control twin, making decisions based on the digital plant state. Combined with RLS online re-identification (Chen et al., 2026c), this constitutes a self-calibrating digital twin (Grieves & Vickers, 2017; Rasheed et al., 2020) that continuously updates its model to match the physical system.

__7.8  Community Adoption Roadmap__

Target users: (1) irrigation district engineers deploying canal automation, (2) water utility automation teams upgrading SCADA systems, (3) university courses in water systems control. Planned outreach: 4 tutorial videos (HydroComponents basics, HydroRTP DMPC, integrated pipeline, PLC deployment), annual CHS Toolchain Workshop at EWRI/IAHR conferences, and a community forum for user support and contribution.
...
These results establish the CHS integrated toolchain as a practical engineering methodology for canal automation, providing a replicable and open-source pipeline from hydraulic model to verified autonomous controller.
