...
3. **Multi-timescale ODD definition**: A 53-constraint ODD across 24 state dimensions, with timescale-specific safety zones for each HDMPC layer and inter-layer consistency constraints.
...
The consensus tolerance $\epsilon_i \leq 0.02$ m is achieved within 3 communication rounds per Segment Agent sampling step, using the Gauss-Seidel decomposition strategy validated by Huang et al. (2026b). Importantly, the consensus algorithm operates on the internal prediction model, not on the physical system: the Family α transport delay ($e^{-\tau_d s}$) is explicitly modeled within the prediction horizon, so consensus convergence depends on model coupling strength $\kappa$ (satisfying $\kappa < 0.25$ for all Jiaodong segments) rather than on the physical delay magnitude (Negenborn et al., 2008).
...
| Solver | MIQP/QP (< 50 ms)† | QP via ParaQP (< 200 ms) | LP/QP (< 5 s) |

__3.5  Communication Architecture__

The 302 km system uses a fiber-optic backbone (OPGW cable along the pipeline right-of-way) with 4G/LTE cellular backup. Redundant OPC UA servers are deployed at S1 (intake) and S5 (midpoint) for System Agent communication. Station-to-Station communication uses Profinet over fiber (< 10 ms latency); System Agent communication uses OPC UA with TLS 1.3 encryption and certificate-based mutual authentication (IEC 62443 Security Level 2) (< 100 ms latency, < 200 ms via cellular backup). Profinet communication is isolated on a dedicated VLAN with firewall-protected gateway to the OPC UA network. Anomaly-based intrusion detection monitors System Agent commands for rate-of-change violations and physical consistency (e.g., setpoints outside actuator ranges are rejected by Segment Agent local constraints). Communication infrastructure annual uptime: 99.97% based on 2023–2025 operational records. During the 30-minute communication loss scenario (S9), the affected Segment Agents operate autonomously using the last-received setpoints and local measurements—a natural fallback enabled by the distributed architecture.

__3.6  Stability Analysis__

**Station Agent (Layer 1)**: Between pump switching events (minimum 30-minute interval), the Station Agent solves a continuous QP in speed ratios $\omega_{i,j}$ with guaranteed stability via terminal cost formulation (Rawlings et al., 2017, Theorem 2.19). Pump switching events are analyzed as bounded impulse disturbances ($|\Delta Q| \leq Q_{\text{unit,max}}$) that settle within 3–5 Station Agent steps (6–10 s), well within the Segment Agent's 60 s interval. This "switching-as-disturbance" treatment is valid because switching frequency is low (at most once per 30 min) relative to the 2 s control period.

**Segment Agent (Layer 2)**: The Family α integrator dynamics ($1/(A_s s)$) are marginally unstable in open loop. Closed-loop stability is guaranteed by: (i) integral action inherent in the tracking MPC formulation, (ii) the check gate providing a second degree of freedom for disturbance rejection, and (iii) the terminal penalty $V_f(\mathbf{h}) = \mathbf{h}^T \mathbf{P} \mathbf{h}$ where $\mathbf{P}$ satisfies the discrete-time algebraic Riccati equation for the linearized Family α model (Maciejowski, 2002, §3.5; Rawlings et al., 2017, §2.4).

**System Agent (Layer 3)**: The hourly LP/QP operates on an aggregated pipeline model with inherent volume-balance constraints, ensuring that cumulative delivery targets are met. Stability at this timescale is enforced by the delivery volume constraints (Eq. 8a) rather than by dynamic stabilization.

**Inter-layer stability**: The singular perturbation argument (Christofides et al., 2002) applies because: (a) each layer's closed-loop dynamics settle well within the next-slower layer's sampling interval (Layer 1: 6–10 s settling vs. 60 s Layer 2 period; Layer 2: 3–5 min settling vs. 3600 s Layer 3 period), and (b) the coupling strength between adjacent segments satisfies $\kappa < 0.25$ (verified via eigenvalue analysis of the Jacobian coupling matrix), ensuring that the Gauss-Seidel consensus converges within the prediction model independently of the physical transport delay.
...
**Environmental flow constraint** (1): Minimum environmental flow at the Mihe River confluence (offtake 4): $Q_{\text{env}} \geq 2.0$ m³/s year-round (Shandong Water Resources Bureau, 2022 regulation). During low-flow periods, the System Agent prioritizes environmental flow over energy optimization.
...
1. **Constraint guarantees**: The HDMPC's ODD-based constraint enforcement ensures that water levels, flows, and pump operating points remain within safe bounds at all times. RL approaches typically handle constraints through penalty terms, which cannot guarantee zero violation—a critical requirement for infrastructure safety.

2. **Interpretability**: At WSAL-2, human operators must understand and approve the controller's recommendations. HDMPC decisions are interpretable through the cost function weights, constraint margins, and predicted trajectories. RL policies are opaque, potentially undermining operator trust.

3. **Sample efficiency**: Training RL policies for a 302 km pipeline requires extensive simulation data (typically $10^5$–$10^6$ episodes). The Family α transfer function model provides a physics-based predictor that requires only system identification (Table 2), not training.
...
__*Table 12. Energy Savings vs. Demand Forecast Accuracy*__

| Demand Forecast MAPE | Energy Savings (%) | Weekly Energy (MWh) | Delivery Deviation (%) |
|---------------------|--------------------|--------------------|-----------------------|
| 0% (perfect) | 7.2 | 4,473 | ±1.1 |
| 5% | 6.9 | 4,488 | ±1.4 |
| 10% | 6.5 | 4,507 | ±1.8 |
| 15% | 5.8 | 4,541 | ±2.4 |
| 20% | 5.1 | 4,575 | ±3.1 |
...
**Phase 1 (months 1–3)**: Layer 1 Station Agents deployed at S1 (intake) and S4 (highest head) in shadow mode—computing speed recommendations without executing, displayed alongside operator decisions for comparison. Acceptance criteria: RMSE within 120% of SiL prediction, zero false ODD alarms.

**Phase 2 (months 4–6)**: Layer 1 active at all 8 stations (operators confirm pump switching only). Layer 2 Segment Agents activated in shadow mode, providing water level predictions to operators. Acceptance criteria: flow tracking within ±2% of setpoint, operator approval rate > 90%.

**Phase 3 (months 7–9)**: Full three-layer HDMPC active at WSAL-2. System Agent provides hourly schedules; operators review and approve via dashboard. All layers subject to ODD monitoring with automatic fallback. Acceptance criteria: energy savings ≥ 5% over Phase 2 baseline, zero ODD violations, operator satisfaction survey > 80% positive.
...
3. The 53-constraint ODD achieves 100% violation detection with zero false alarms across 12 of 14 scenarios, with the two violations in S12 (source shutdown) correctly triggering the VIOLATION fallback protocol.
