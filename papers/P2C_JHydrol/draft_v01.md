...
The SiL platform uses the cuSVE GPU-parallel Saint-Venant equation solver (Huang et al., 2026a) for real-time hydraulic simulation of the 302 km pipeline. The cuSVE model is discretized into 604 computational cells (500 m each, $\Delta x = 500$ m) and validated against historical SCADA data ($R^2 = 0.96$ for water levels, $R^2 = 0.93$ for flows). The HDMPC controllers run on a Beckhoff CX2062 industrial PC (Intel Xeon E-2176G, 6 cores, 3.7 GHz) communicating with cuSVE via shared memory, achieving end-to-end control loop latency < 15 ms.
...
__6.2  Three-Layer vs. Two-Layer HDMPC__
...
__6.4  Computational Requirements__
...
__6.5  Limitations and Future Work__
...
Wang, H., Zhang, C., Li, X., & Zhou, J. (2022). Deep reinforcement learning for real-time operation of large-scale water transfer systems. Water Resources Research, 58(8), e2021WR031380.
...
Zheng, F., Simpson, A. R., Zecchin, A. C., & Deuerlein, J. W. (2013). A graph decomposition-based approach for water distribution network optimization. Water Resources Research, 49(4), 2093–2109.
