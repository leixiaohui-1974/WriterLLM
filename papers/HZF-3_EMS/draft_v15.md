...
For each release, `cross_paper_consistency_hzf3.json.consistency_checks[*].evidence` must reference the current draft version (e.g., `draft_v15.md`) rather than older snapshots, ensuring version-synchronous audit trails.
...
The consistency artifact must include `current_draft: "draft_v15.md"`, and all version-sensitive checks (including evidence freshness) must resolve against this pointer to avoid stale literal references after version bumps.
