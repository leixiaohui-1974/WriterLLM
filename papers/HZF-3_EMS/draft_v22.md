...
## 0.1 Key claims and evidence status (v22)

| Claim | Type | Evidence status in v22 |
|---|---|---|
...
For each release, `cross_paper_consistency_hzf3.json.consistency_checks[*].evidence` must reference the current draft version (e.g., `draft_v22.md`) rather than older snapshots, ensuring version-synchronous audit trails.
...
For deterministic machine replay, `submission_readiness_gate.json.gates` must keep a stable order across versions: `evidence_monitor_pass` -> `governance_policy_validated` -> `terminology_lock_p1a_p2a` -> `audit_artifacts_consistent` -> `consistency_evidence_freshness` -> `external_reference_verification`.
...
The consistency artifact must include `current_draft: "draft_v16.md"`, and all version-sensitive checks (including evidence freshness) must resolve against this pointer to avoid stale literal references after version bumps.
...
### 7.11 Fingerprint canonicalization rule (v22)
