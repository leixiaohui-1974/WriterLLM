...
## 0.1 Key claims and evidence status (v10)

| Claim | Type | Evidence status in v10 |
|---|---|---|
...
For each release, `cross_paper_consistency_hzf3.json.consistency_checks[*].evidence` must reference the current draft version (e.g., `draft_v20.md`) rather than older snapshots, ensuring version-synchronous audit trails.
...
### 7.11 Fingerprint canonicalization rule (v20)
