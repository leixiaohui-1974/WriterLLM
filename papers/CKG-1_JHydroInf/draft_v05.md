...
| System | States | Actuators | Agents | MPC Dim | FMU Size | Hardware Cost | MBD Challenge |
|--------|--------|-----------|--------|---------|----------|---------------|---------------|
| Dual\-tank \(this paper\) | 4 | 1 | 1 | 6×4 QP | 0\.5 MB | ~\$200 | Proof\-of\-concept |
| 3\-pool canal \(Liu et al\., 2026\) | 63 | 3 | 3 | 20×5 QP ×3 | 5 MB | ~\$5k | DMPC coordination |
| 10\-pool system | 210 | 10 | 10 | Multi\-agent DMPC | 15 MB | ~\$50k | Communication topology, ODD explosion |
| Field system \(100\+ structures\) | 1000\+ | 50\+ | 50\+ | Hierarchical DMPC | 100\+ MB | \$500k\+ | Multi\-FMU co\-sim performance, PiL |

Note: Hardware costs are order\-of\-magnitude estimates for instrumentation and control hardware only, excluding engineering labor and software licenses\.
...
1\. The complete MBD pipeline—from OpenModelica physical model \(Layer 7\) through IDZ control model \(Layer 6\), MPC design, automatic code generation \(Layer 3\), and Arduino deployment \(Layer 2\)—produces controllers that meet all WSAL\-2 acceptance criteria on physical hardware: water level RMSE of 2\.0 mm \(mean\) and 3\.8 mm \(worst case\) with Kalman filtering, and zero constraint violations across all 16 ODD scenarios\.
