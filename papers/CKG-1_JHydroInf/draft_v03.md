...
A Kalman filter based on the IDZ model serves as the state estimator, reducing effective measurement noise from $\sigma = 0.8$ mm \(raw ultrasonic sensor\) to $\sigma_{est} = 0.3$ mm\. The Kalman filter uses the IDZ model as the prediction model with process noise covariance $Q_k = 0.01 I$ and measurement noise covariance $R_k = 0.64 \times 10^{-6}$ \(corresponding to $\sigma = 0.8$ mm\)\.
