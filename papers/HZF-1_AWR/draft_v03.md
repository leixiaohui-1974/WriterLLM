...
The solver accepts input via a JSON network description file specifying pool geometry, actuator parameters, boundary conditions, and simulation parameters. Output is written to HDF5 format or streamed via shared memory for real-time coupling.
...
- __Hardware required__: NVIDIA GPU with CUDA Compute Capability ≥ 7.0 (Volta or later)
